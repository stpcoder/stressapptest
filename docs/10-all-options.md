...
이 장에서는 `Sat::ParseArgs()`가 실제로 인식하는 옵션을 기능별로 정리합니다. 프로그램의 help와 실제 parser 사이에 철자 차이가 있는 옵션도 별도로 표시합니다.
...
## Cache coherency와 CPU frequency 검사
