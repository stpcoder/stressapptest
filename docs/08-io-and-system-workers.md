...
기본 `--non_destructive`는 임의 위치를 읽고 I/O 상태를 확인합니다. `--destructive`는 SAT pattern write와 verify 단계를 추가합니다.
...
**코드 설명:** CPU 연산을 반복하고 매 반복이 끝날 때 scheduler에 다른 thread를 실행할 기회를 줍니다. 이 Worker의 결과는 연산 loop 완료 상태와 실행 시간으로 집계됩니다.
...
- `--cc_line_size`: cache line 크기를 직접 지정
...
**코드 설명:** SoC 오류 확인 함수를 1초 간격으로 호출합니다. 공통 Android·Linux `OsLayer::ErrorPoll()`은 0을 반환합니다. ECC 또는 RAS 수집은 SoC별 `ErrorPoll()` 구현에서 제공합니다.
...
Android ARM의 CPU 주파수는 cpufreq sysfs, tracepoint, SoC 제조사 profiler, simpleperf 또는 Perfetto로 측정합니다. 공통 AArch64 build의 `--cpu_freq_test`는 초기화 단계에서 unsupported 결과를 반환합니다.
...
**코드 설명:** Monitor mode는 `ErrorPollThread`만 실행합니다. 실제 SoC 오류 수집 범위는 target의 `OsLayer::ErrorPoll()` 구현으로 결정됩니다.

`--monitor_mode`는 메모리 부하 Worker 생성을 생략하고 `ErrorPollThread`를 실행합니다.
