...
이 장에서는 stressapptest가 직접 제어하거나 확인할 수 없는 항목을 정리합니다. 특히 LPDDR channel·bank·row mapping과 controller scheduling은 SoC 문서 또는 hardware counter가 필요합니다.
...
## Generic ARM에서 ErrorPoll이 동작하지 않음
...
**해석:** public generic build는 platform-specific subclass를 선택하지 않습니다. ErrorPollThread가 1초마다 실행되더라도 generic `ErrorPoll()`은 hardware incident를 수집하지 않습니다. Android vendor RAS, EDAC 또는 firmware log 연동에는 별도 `OsLayer` 구현이 필요합니다.

## ARM64에서 오류를 다시 읽을 때의 제한
...
## Built-in help와 실제 parser가 다른 부분
