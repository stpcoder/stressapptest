# cache를 거쳐 LPDDR 부하가 만들어지는 과정

stressapptest는 Android의 일반 cacheable memory를 사용합니다. cache를 끄거나 LPDDR을 직접 읽고 쓰지 않습니다. 대신 cache보다 큰 영역을 여러 worker가 계속 처리하여 cache miss와 write-back을 늘립니다.

## stressapptest가 cache를 끄는가?
...
## 오류를 다시 읽을 때 사용하는 `Flush()`의 ARM64 한계
...
## CPU cache와 filesystem page cache의 차이
